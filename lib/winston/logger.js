...
Logger.prototype.log = function (level, msg) {
  ...
  for (var counter_key in this.counters) {
		var counter = this.counters[counter_key];
		var props = {};

		// increment only at levels given by the counter's configured properties AND found in meta
		for (pi in counter.properties) {
		    propkey = counter.properties[pi]; 
		    if (meta[propkey])
			props[propkey] = meta[propkey];
		}
		counter.increment(new Date(), props);
  }; 
