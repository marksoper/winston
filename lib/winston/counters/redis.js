...
var Redis = exports.Redis = function(redis_client,options) {
...
Redis.prototype.increment = function(timestamp) {
	...
	for (li in this.levels) {
  	  rc.incr("allprops||" + dateToTick(timestamp,this.levels[li]));
          //console.log("increment - allprops||" + dateToTick(timestamp,this.levels[li]));
	}
	for (prop in this.properties)
	  rc.incr(prop + "::" + this.properties[prop] + "||alltime");
	for (prop in this.properties) {
  	for (li in this.levels) {
	    var key =  prop + "::" + this.properties[prop] + "||" + dateToTick(timestamp,this.levels[li]);
			rc.incr(key);
		}
	}	
};
