...
var Redis = exports.Redis = function(options,redis_client) {
  ...
  this.prop_time_delimiter = options.prop_time_delimiter || this.prop_time_delimiter;
  this.prop_delimiter = options.prop_delimiter || "__";
  this.time_delimiter = options.time_delimiter || "__";
	this.time_parts_delimiter = options.time_parts_delimiter || "-";
...
//
Redis.prototype.commitToRedis(propkey, propval, timekey, timeval) {
	proptimeval = this.propval + this.prop_time_delimiter + this.timekey + this.time_delimiter + timeval;
  ...
  // actual storage required depends heavily on the cardinality of the actual properties data
	for (li in this.levels) {
		for (prop in properties) {
			this.emit('increment', prop, properties[prop], this.levels[li], helpers.dateToTick(timestamp, this.levels[li], this.time_parts_delimiter));
			this.commitToRedis(prop, properties[prop], this.levels[li], helpers.dateToTick(timestamp, this.levels[li], this.time_parts_delimiter));
		}
	}
...
Redis.prototype.count = function(start_time, end_time, level, properties, callback) {
	...
	for (ti in ticks) {
		...
		for (vi in vals) {
		    var valkey = properties[pi] + this.prop_delimiter + vals[vi] + this.prop_time_delimiter + ticks[ti];
		    keys.push(valkey);
