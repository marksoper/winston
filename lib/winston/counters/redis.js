...
//
Redis.prototype.count = function(start_time, end_time, level, properties) {
	...
	var t = start_time;
	while (t <= end_time) {
		timekey = level + "::" + t[LEVEL_MAP[level]['method']]();
...
dateToTick = function(level,timestamp) {
	var time_vals = [timestamp[LEVEL_MAP[level]['method']]()];
	for (p in LEVEL_MAP[level]['parents']) {
		time_vals.unshift(timestamp[LEVEL_MAP[p]['method']]());
	}
...
nextTickAndTime = function(tick, level) {
	...
	if (level == "year") {
		...
		result['time'] = current_time + LEVELS_MAP[level]['interval'];
	}
...
buildTickSeries = function(start_time, end_time, level) {
  ...
  var ticks = [tick];
	...
	if (["day","hour","minute"].indexOf(level) >= 0) {
		while (t <= end_time) {
			var next_tick_time = nextTickAndTime(tick, level);
			ticks.push(next_tick_time['tick']);
	...
	var tick = start_time[LEVEL_MAP[level]['method']]();
	var ticks = [tick];
	while (t <= end_time) {
