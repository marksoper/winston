...
Redis.prototype.increment = function(timestamp,properties) {
	...
	for (prop in this.properties) {
	  ...
	  this.redis_client.sadd(prop,this.properties[prop]);
	  console.log("Increment adding to set: " + prop + " value: " + this.properties[prop]);
	}
	for (prop in this.properties) {
  	for (li in this.levels) {
...
Redis.prototype.count = function(start_time, end_time, level, properties, callback) {
	...
	for (ti in ticks) {
		...
		var vals = this.redis_client.smembers(properties[pi]);
		for (val in vals) {
		    var valkey = properties[pi] + "::" + val + "||" + ti;
		    keys.push(valkey);
