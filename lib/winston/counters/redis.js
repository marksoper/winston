...
Redis.prototype.commitToRedis = function(propkey, propval, timekey, timeval) {
        proptimeval = String(propval) + helpers.PROP_TIME_DELIMITER + timekey + helpers.TIME_DELIMITER + String(timeval);
	this.redis_client.hincrby(propkey, proptimeval,1, function(error, reply) {
		console.log("  REDIS HASH@  " + propkey + "  REDIS VAL@  " + helpers.PROP_DELIMITER + proptimeval + "  REDIS REPLY@  " + reply + "  ERROR@ " + error);
	    });
};
...
Redis.prototype.increment = function(timestamp,properties) {
	...
	for (pr in properties) {
	    console.log(pr + " : " + properties[pr]);
	}

  // increment and emit for each property for each time level
  // emits events with 4 params: propkey, propval, timekey, timeval 
  //NOTE: Requires at most (T+1) * (P+1) units of storage
