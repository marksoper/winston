...
var PROP_TIME_DELIMITER = exports.PROP_TIME_DELIMITER = "||";
var PROP_DELIMITER = exports.PROP_DELIMITER = "__";
var TIME_DELIMITER = exports.TIME_DELIMITER = "__";
var TIME_PARTS_DELIMITER = exports.TIME_PARTS_DELIMITER = "-";


var dateToTick = exports.dateToTick = function(timestamp,level) {

  // timestamp not actually of type Date so just return it as the time value 
  if (level == "alltime")
    return "alltime";
  ...
  var tick_time = time_vals.join(TIME_PARTS_DELIMITER);
  return tick_time; 

};

var determinePrecision = exports.determinePrecision = function(tick) {
	return tick.split(TIME_DELIMITER)[0];
};


var tickToDate = exports.tickToDate = function(tick) {
	var tick_parts = tick.split(TIME_PARTS_DELIMITER);
	var level = tick_parts[0];
	var time_parts = tick_parts[1].split(TIME_PARTS_DELIMITER);
        for (var i=0; i<=5; i++) {
	    if (!time_parts[i])
	      time_parts[i] = 0;
	}
